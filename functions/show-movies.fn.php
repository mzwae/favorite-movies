...
function showMovies($data) {
    ...
    switch($data) {
        ...
        case "favs":
            $stmt = $db->prepare("SELECT * FROM movies WHERE movie_id IN (SELECT movie_id FROM favourites WHERE favourites.user_id = ?) ORDER BY title");
            $stmt->bind_param('i', $userID);
            break;

        
        case "non_favs":
            $stmt = $db->prepare("SELECT * FROM movies WHERE movie_id NOT IN (SELECT movie_id FROM favourites WHERE favourites.user_id = ?) ORDER BY title");
            $stmt->bind_param('i', $userID);
            break;
